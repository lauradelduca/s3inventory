## Preprocessing of Mexico customs declarations trade data 2013 - 2017 from SICEX2.5 (codes 01-99)
...
for (yy in 2013:2017){
	...
	orig <- get_bucket_df(	bucket = 'trase-storage', prefix = paste0('data/1-TRADE/CD/EXPORT/MEXICO/', yy))	
	...
	write.table(	D, 
					paste0(current_folder, '/', 'CD_MEXICO_', yy, '_TEST.csv'), 
					quote = FALSE, 
					row.names = FALSE, 
					dec = '.', 
					sep = ';')
	...
	put_object(	file = rawConnectionValue(zz), 
				...
				object = paste0('data/1-TRADE/CD/EXPORT/MEXICO/', yy, '/SICEX25/CD_MEXICO_', yy, '.csv') )
