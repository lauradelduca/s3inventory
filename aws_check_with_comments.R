...
# include check with row== weight 0
...
for (f in as.vector(CD$file)){
	...
	if (CD$country[CD$file == f] == 'ARGENTINA'){
		...
		weight <- data.frame(commodity = as.integer(substr(data$HARMONIZED_CODE_PRODUCT_ENGLISH, 1, 6)), weight = as.numeric(data$TOTAL_NET_WEIGHT_KG))
