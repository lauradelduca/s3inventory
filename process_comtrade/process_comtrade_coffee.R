# prepare COMTRADE data for coffee
...
hs6 <- as.vector(as.numeric(hs$code_value[hs$com_name == 'COFFEE']))
...
for (f in ff){
	...
	write.csv2(j, paste0(f, '_coffee', '.csv'), quote = FALSE, row.names = FALSE)
